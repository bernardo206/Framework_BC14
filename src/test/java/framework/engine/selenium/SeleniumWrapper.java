...
public class SeleniumWrapper {
    ...
    public String isDisplayed3(By locator) {
        return (String) ((JavascriptExecutor) driver).executeScript("alert('Se confirma la existencia de Texto: -Tus fechas de Búsqueda están en el pasado- ');");
...
*/
    ...
    public void identificarPestaniasOfertasRenfeTrenes() {

        // Obtén los identificadores de las ventanas abiertas
        Set<String> windowHandles = driver.getWindowHandles();
        System.out.println(windowHandles);

        List<String> windowHandlesList = new ArrayList<>(windowHandles); // Convertir el Set en una lista

        if (windowHandlesList.size() > 1) {
            driver.switchTo().window(windowHandlesList.get(1)); // Acceder a la segunda pagina
            System.out.println("acceso a pag 2");

            //Interactuar en segunda pagina
            String texto1 = obtenerTexto(By.xpath("//div[contains(@class, 'NoResultsPanel__Title') and contains(text(), 'Tus fechas de b')]"));

            if (texto1.contains("Tus fechas de b")) {
                isDisplayed3(By.xpath("//div[contains(@class, 'NoResultsPanel__Title') and contains(text(), 'Tus fechas de b')]" ));
                System.out.println("Confirmación de existencia de texto solicitado ");
            } else {
                System.out.println(" ");
            }

        } else {
            System.out.println("No hay suficientes ventanas para cambiar.");
        }
    }
